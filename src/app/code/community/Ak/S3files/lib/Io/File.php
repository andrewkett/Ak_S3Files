 ...
 * File upload class
 *
 * When the Ak_S3files extension is enabled this file will be loaded in place of the lib version of Varien_Io_File
...
class Varien_Io_File extends Varien_Io_Abstract
{
    ...
    {
        Mage::log("rmdir ".$dir, null, 'filesystem.log');
    ...
    {
        Mage::log("rmdir recursive  ".$dir, null, 'filesystem.log');
    ...
    {
        Mage::log("cd ".$dir, null, 'filesystem.log');
//        Mage::log("rp dir is: ".realpath($dir), null, 'filesystem.log');
...
//        }
    ...
    {
        Mage::log("read ".$filename, null, 'filesystem.log');
        if (!is_null($dest)) {
            chdir($this->_cwd);
    ...
    {
        Mage::log("write to ".$filename.' from '.$src, null, 'filesystem.log');
    ...
    {
        Mage::log("_IsValidSource ".$src, null, 'filesystem.log');
    ...
    {
        Mage::log("_isFilenameWriteable ".$filename, null, 'filesystem.log');
        $error = false;
        @chdir($this->_cwd);
    ...
    {
        Mage::log("_checkSrcIsFile ".$src, null, 'filesystem.log');
    ...
    {
        Mage::log("filePutContent ".$filename. 'from '.$src, null, 'filesystem.log');
    ...
    {
        Mage::log("fileExists ".$file, null, 'filesystem.log');
    ...
    {
        Mage::log("isWriteable ".$path, null, 'filesystem.log');
    ...
    {
        Mage::log("checkAndCreateFolder ".$folder, null, 'filesystem.log');
    ...
    {
        Mage::log("rm ".$filename, null, 'filesystem.log');
        //die($filename);
        @chdir($this->_cwd);
    ...
    {
        Mage::log("mv ".$src.' to '.$dest, null, 'filesystem.log');
        //chdir($this->_cwd); @changed
        $result = @rename($src, $dest);
        //chdir($this->_iwd); @changed
        return $result;
    }
    ...
    {
        Mage::log("cp ".$src.' to '.$dest, null, 'filesystem.log');
    ...
    {
        Mage::log("chmod ".$filename.' to '.$mode, null, 'filesystem.log');
    ...
    {
        Mage::log("ls ".$grep, null, 'filesystem.log');
